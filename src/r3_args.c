...
int     g_option_context_parse (GOptionContext *context, int *argc, char***argv, void* error)
{
    ...
    for (i=1; i<*argc; i++)
    {
        ...
        if (arg[0] == '-'){
            ...
            {
                int len = strlen(entry->long_name);
                int short_name =  arg[1] == entry->short_name && arg[2] == '\0';

                if (short_name || (arg[1] == '-' && strncmp(entry->long_name, arg+2, len)==0))
                {
                    switch(entry->arg){
                    ...
                    case G_OPTION_ARG_INT:
                        if (arg[2+len]=='=') arg+=3+len;
                        else arg = args[++i];
                        if (entry->arg_data!=NULL && i<*argc) {
                            if (arg[0]=='0' && arg[1]=='x') 
                                *(uint64_t*)entry->arg_data = strtoull(arg, NULL, 16);
                            else
                                *(int*)entry->arg_data = atoi(arg);
                        }
                        break;
                    case G_OPTION_ARG_DOUBLE:
                        ...
                        if (entry->arg_data!=NULL && i<*argc)
                            *(double*)entry->arg_data = strtod(arg, NULL);
                        break;
