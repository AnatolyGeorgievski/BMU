...
#include <stddef.h>
/* Сборка 
	$ gcc -DTEST_CRC5 -o crc5 crc5.c sha256.c stratum_test.c
 */
/*
CRC-16/CCITT-FALSE
...
uint8_t crc5(const uint8_t *ptr, size_t bits)
{
	//crc<<=3; //
	uint8_t crc = (0x1F<<3);
	int i;
...
uint8_t crc8(uint8_t crc, const uint8_t *ptr, size_t len) {
	while(len--){
		crc = CRC8_Lookup[crc^(*ptr++)];
	}
	return crc;
}
uint16_t crc15_can(uint16_t crc, const uint8_t *ptr, size_t len)
{
	crc<<=1;
	while(len--){
		crc^= ((*ptr++)<<8);
...
uint16_t crc16_itu(uint16_t crc, const uint8_t *buffer, size_t len)
{
	while(len--)
	{
...
static const uint16_t CRC16M_lookup4[] ={
	0x0000, 0xCC01, 0xD801, 0x1400, 0xF001, 0x3C00, 0x2800, 0xE401,
	0xA001, 0x6C00, 0x7800, 0xB401, 0x5000, 0x9C01, 0x8801, 0x4400,
};
uint16_t crc16m(uint16_t crc, const uint8_t *buffer, size_t size)
{
	if (size) do{
		crc^= *(uint8_t*)buffer++;
		crc = (crc>>4) ^ CRC16M_lookup4[crc & 0xF];
		crc = (crc>>4) ^ CRC16M_lookup4[crc & 0xF];
	} while (--size);
	return (crc);
}

#if defined(TEST_CRC16)
#include <stdio.h>
...
int main(int argc, char* argv[]){
	crc_gen_inv_table(0xA001,16);
	uint8_t test[] = "123456789";
	uint16_t crc = crc16m(0xFFFF, test, 9);
	printf("CRC = %04X ..%s\n", crc, crc==CRC16M_CHECK?"ok":"fail");
	return 0;
}
#endif